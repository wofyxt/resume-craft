...
const ResumeList = ({ userId }) => {
  ...
  if (resumes.length === 0) return <p>У вас пока нет резюме. <button onClick={() => navigate('/editor/new')} className="btn btn-outline">Создать первое</button></p>;

  return (
    <>
      {resumes.map(r => (
        <div key={r.id} className="resume-card" onClick={() => handleEdit(r.id)} style={{ cursor: 'pointer' }}>
          <div className="resume-info">
            <h3>{r.title}</h3>
            <p>Шаблон: {r.template} • Обновлено: {new Date(r.updatedAt).toLocaleDateString('ru-RU')}</p>
          </div>
          <div className="resume-actions">
            <button className="btn-sm" onClick={(e) => { e.stopPropagation(); handleEdit(r.id); }}>
